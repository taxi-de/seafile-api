...
module SeafileApi
  class Connect
    ...
    def put_new_meber_to_group(user_email, group_id)
      JSON.parse(curl_put("#{host}/api2/groups/#{group_id}/members/", { 'user_name' => user_email }).body_str)
    end

    def delete_group_member(user_email, group_id)
      JSON.parse(curl_delete("#{host}/api2/groups/#{group_id}/members/", { 'user_name' => user_email }).body_str)
    end
  end
end
