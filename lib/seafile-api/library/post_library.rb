...
module SeafileApi
  class Connect
    ...
    def post_default_lib
      JSON.parse(curl_post("#{host}/api2/default-repo/").body_str)
    end
    ...
    def post_public_lib(repo_id)
      curl_post("#{host}/api2/repos/#{repo_id}/public/").body_str
    end
    ...
    def post_lib(data)
      JSON.parse(curl_post("#{host}/api2/repos/", data).body_str)
    end
    ...
    def post_lib_pass(repo_id, data)
      curl_post("#{host}/api2/repos/#{repo_id}/", data).body_str
    end
  end
end
