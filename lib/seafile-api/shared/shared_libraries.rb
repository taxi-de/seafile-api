...
module SeafileApi
  class Connect
    ...
    def put_share_library(repo_id, share_type, user, group_id, permission)
      curl_put("#{host}/api2/shared-repos/#{repo_id}/?share_type=#{share_type}&user=#{user}group_id=#{group_id}&permission=#{permission}").body_str
    end
    ...
    def delete_unshare_library(repo_id, share_type, user, group_id)
      curl_delete("#{host}/api2/shared-repos/#{repo_id}/?share_type=#{share_type}&user=#{user}group_id=#{group_id}").body_str
    end
