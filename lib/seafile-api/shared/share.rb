...
module SeafileApi
  class Connect
    ...
    def delete_share_link(shared_t)
      curl_delete("#{host}/api2/shared-links/?t=#{shared_t}").head.split[2]
    end
    ...
    def create_share_link(file, repo_id)
      curl_put("#{host}/api2/repos/#{repo_id}/file/shared-link/", { 'p' => file }).head.split('Location:')[1].scan(%r{\w{4,5}://.{1,}/})[0]
    end
  end
end
