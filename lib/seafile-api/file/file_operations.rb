...
module SeafileApi
  class Connect
    ...
    def delete_file(filename, repo)
      curl_delete("repos/#{repo}/file/?p=#{filename}").body_str
    end
    ...
    def post_revert(filename, commit_id, repo)
      curl_put("repos/#{repo}/file/revert", { 'commit_id' => commit_id, 'p' => filename }).body_str
    end
    ...
    def post_file(filename, repo)
      curl_post("repos/#{repo}/file/?p=#{filename}", { 'operation' => 'create' }).body_str
    end
    ...
    def post_move(filename, src_repo, data)
      curl_post("repos/#{src_repo}/file/?p=#{filename}", data).body_str
    end
    ...
    def post_copy(src_repo, data)
      curl_post("repos/#{src_repo}/fileops/copy/", data).body_str
    end
    ...
    def post_rename(old_name, repo, data)
      curl_post("repos/#{repo}/file/?p=#{old_name}", data).body_str
    end
  end
end
