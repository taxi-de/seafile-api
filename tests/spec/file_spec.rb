...
describe 'SeafileApi::Connect' do
  ...
  describe 'work with file' do
    ...
    it 'should return direct link' do
      expect(SF.file_link('test.pdf')).to match(%r{/files/})
    end
    ...
    describe 'and history' do
      ...
      it 'should return link to specific revision of file' do
        expect(SF.link_revision('test.pdf', @ret['commits'][0]['id'])).to match(%r{/files/})
      end
    end

    it 'should return block link' do
      expect(SF.block_link).to match(%r{/upload-blks-api/})
    end
    ...
    xit 'should copy file' do
      expect(SF.copy_file('simple.txt', '/111')).to match(%r{/upload-blks-api/})
    end
